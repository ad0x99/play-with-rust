...
impl Page for HomePage {
    fn draw_page(&self) -> Result<()> {
        ...
        for id in epics.keys().sorted() {
            let epic = epics.get(id).unwrap();
            let id_column = get_column_string(&id.to_string(), 11);
            let name_column = get_column_string(&epic.name, 32);
            let status_column = get_column_string(&epic.status.to_string(), 17);
    ...
    fn handle_input(&self, input: &str) -> Result<Option<Action>> {
        ...
        // Match the input against the possible actions
        match input {
            "q" => Ok(Some(Action::Exit)),
            "c" => Ok(Some(Action::CreateEpic)),
            input => {
                // Check if the input is a valid epic id
...
impl Page for EpicDetail {
    fn draw_page(&self) -> Result<()> {
        ...
        let id_column = get_column_string(&self.epic_id.to_string(), 5);
        let name_column = get_column_string(&epic.name, 12);
        let description_column = get_column_string(&epic.description, 27);
        let status = get_column_string(&epic.status.to_string(), 13);
        println!(
            "{} | {} | {} | {}",
            id_column, name_column, description_column, status
        );
        ...
        for id in epic.stories.iter().sorted() {
            ...
            let id_column = get_column_string(&id.to_string(), 11);
            let name_column = get_column_string(&story.name.to_string(), 32);
            let status = get_column_string(&story.status.to_string(), 17);
            println!("{} | {} | {}", id_column, name_column, status);
        }
...
impl Page for StoryDetail {
    fn draw_page(&self) -> Result<()> {
        ...
        let id_column = get_column_string(&self.story_id.to_string(), 5);
        let name_column = get_column_string(&story.name, 12);
        let description_column = get_column_string(&story.description, 27);
        let status = get_column_string(&story.status.to_string(), 13);
        println!(
            "{} | {} | {} | {}",
            id_column, name_column, description_column, status
        );
